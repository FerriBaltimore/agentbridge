...
class MessageSubmissionMixin:
    def message_create(self, instance_id, content, *, model=None, effort=None, context_window=None,
        ...
        policy = previous_message_policy(self.store, idempotency_key, instance_id) if delivery == 'reject' else None
        policy = policy or session
        options = RunOptions(
            timeout=(timeout_ms / 1000) if timeout_ms is not None else 600,
            sandbox=policy['sandbox_mode'] if sandbox_mode is _DEFAULT else sandbox_mode,
            permission_mode=policy['permission_mode'] if permission_mode is _DEFAULT else permission_mode,
        ...
        if delivery != 'reject':
            return self._queue_submit(
                ...
                expected_instance_version=session['version'])
