...
class InstanceRoutingMixin:
    def instance_create(self, *, engine=None, account_ref=None, provider=None,
        ...
        policy = validate_policy(permission_mode, sandbox_mode)
        if effort or context_window or allowed_tools:
            raise UnsupportedError('Effort, context window and tool controls are configured per turn.')
        ...
        if mode == 'pinned':
            ...
            result = self.session(account.id, workspace_path or '.', model=model,
                                  request_key=idempotency_key, evaluation=evaluation, **policy)
        ...
        return create_automatic_instance(
            ...
            excluded_account_refs=excluded_account_refs, initial_account=initial_account, **policy)
