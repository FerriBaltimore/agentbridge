...
def dispatch(bridge,method,params):
    ...
    if method=='messages.get':return bridge.message_get(**params)
    if method=='queues.list':return bridge.queue_list(**params)
    if method=='queues.add':return bridge.queue_add(**params)
    if method=='queues.move':return bridge.queue_move(**params)
    if method=='queues.delete':return bridge.queue_delete(**params)
    if method=='queues.dispatch':return bridge.queue_dispatch(**params)
    if method=='queues.pause':return bridge.queue_pause(**params)
    if method=='queues.resume':return bridge.queue_resume(**params)
