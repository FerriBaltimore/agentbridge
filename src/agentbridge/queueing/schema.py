...
def migrate_v10(db, version):
    ...
    if current != 9:
        db.commit()
        return current
    db.execute('''CREATE TABLE IF NOT EXISTS conversation_queues(
    ...
    db.execute('''CREATE TABLE IF NOT EXISTS queued_messages(
    ...
    db.execute('''CREATE INDEX IF NOT EXISTS queued_message_order
            ON queued_messages(session_id,state,position)''')
    db.execute('UPDATE metadata SET version=10')
    db.commit()
    return 10
