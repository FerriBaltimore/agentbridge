...
class QueueMixin:
    ...
    def _queue_submit(self, instance_id, prompt, options, *, execution, exclusions,
                      ...
                      inherit_permissions, inherit_sandbox, expected_instance_version=None):
        identifier(instance_id)
        ...
        # Hash omission intent, not mutable defaults; the saved options remain frozen.
        if inherit_permissions:
            raw['options']['permission_mode'] = 'dontAsk'
        if inherit_sandbox:
            raw['options']['sandbox'] = 'read-only'
        if delivery != 'steer' and (inherit_permissions or inherit_sandbox):
            raw['instance_policy_inheritance'] = [inherit_permissions, inherit_sandbox]
        digest = sha256(dumps(raw).encode()).hexdigest()
        legacy_raw = {key: value for key, value in raw.items() if key != 'instance_policy_inheritance'}
        legacy_digest = sha256(dumps(legacy_raw).encode()).hexdigest()
        queue = QueueStore(self.store)
        previous = queue.replay(idempotency_key, digest, legacy_digest=legacy_digest)
        if previous:
