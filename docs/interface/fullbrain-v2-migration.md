...
immutable AgentBridge wheel and its pinned Codex, GrantBridge, Node.js and
CLIProxyAPI contents before changing Fullbrain's worker. A method in the target
[operations](operations.md) list is
...
environment on Linux x86_64 or ARM64. The platform wheel carries pinned
CLIProxyAPI, Codex, the GrantBridge proxy adapter and Node.js. No external
binary path variable is required. Its source build verifies pinned downloads;
the installed wheel verifies and extracts its runtime offline into AgentBridge's
private state root outside the model-writable workspace. Managed sidecars
require Linux `memfd`, a dedicated loopback endpoint and an auth directory for
each account. Their keys
...
Replacing the wheel does not replace an already-running account sidecar;
