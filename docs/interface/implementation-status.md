...
| Managed sidecar | AgentBridge prepares one loopback CLIProxyAPI process and auth directory per account; the proxy executable must be installed or supplied via `AGENTBRIDGE_CLIPROXY_BIN` | Linux `memfd` is required; local lifecycle checks do not establish live provider acceptance |
| Auth custody | GrantBridge coordinates the sidecar Management API browser flow; CLIProxyAPI owns upstream credential and refresh; managed local keys remain transient in supervisor memory | No credential values or raw provider errors in AgentBridge records; live refresh pending |
