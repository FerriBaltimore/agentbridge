...
| Execution access | Persistent read-only, workspace-write or full-access policy; per-turn overrides | Real subprocess access and browser fixtures; live provider acceptance separate |
...
acceptance is pending. Instance permission and sandbox defaults are supported;
see [execution access](execution-access.md). Instance effort/context defaults, `allowed_tools` and
